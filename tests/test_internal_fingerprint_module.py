...
from internal.intel.fingerprint import (
...
def test_internal_parse_http_server_header_normalizes_spacing() -> None:
    assert parse_http_server_header("  nginx/1.25.3   ") == "nginx/1.25.3"


def test_internal_parse_html_title_and_metadata_extracts_title_and_meta() -> None:
...
def test_internal_detect_device_model_hint_matches_known_patterns() -> None:
    model = detect_device_model_hint("Edge camera :: Axis P3225-LV network camera")

    assert model == "Axis P3225-LV"


def test_internal_detect_firmware_version_hint_matches_common_formats() -> None:
    version = detect_firmware_version_hint("generator=Firmware version 10.12.3")

    assert version == "10.12.3"


def test_internal_build_web_fingerprint_combines_all_web_signals() -> None:
